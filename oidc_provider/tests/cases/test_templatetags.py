...
class LogoutClientsTagTest(TestCase):
    TEMPLATE = Template(
        "{% load frontchannel_logout %}{% logout_clients user_logged_out=user_logged_out %}"
    )
    ...
    def test_render_logout_clients_iframe(self):
        ...
        self.assertIn(
            'iframe src="http://example.com/logout" data-oidc-logout',
            rendered,
        )
        self.assertIn(
            'iframe src="http://example2.com/logout?iss=http://localhost:8000/openid&sid=',
            rendered,
        )
